...
import { captionCueAt, drawCaption, captionFontPx, drawWatermark, drawCornerLogo } from "./captions";
import { drawTextOverlays } from "./textOverlay";
import { transitionOf } from "./transitions";
import { applyFx, fxSeed } from "./imageEffects";
import { createVideoSource } from "./videoDecodeSource";
import { createVoiceSource } from "./voiceSource";
import { createVoiceFxProcessor } from "./voiceFx";
...
// CRF-23-equivalent target bitrate for the given resolution/fps. The WebCodecs
// path can't run true CRF on every browser (only the quantizer mode Chrome
// offers), so the rest encode at a fixed rate — use a rate that lands on roughly
// the same file size ffmpeg `-crf 23` produced for slideshow content (the
// pre-WebCodecs default), instead of a blanket 8 Mbps that bloats 12-min videos
// to ~700 MB.
export function crfTargetBitrate(width, height, fps = 30) {
  const px = (width || 1920) * (height || 1080);
  // Scales with pixels × fps so 4K/60 isn't starved; ~0.6…40 Mbps covers
  // 720p slideshows up to 4K UHD.
  return Math.max(1_200_000, Math.min(40_000_000, Math.round(px * fps * 0.045)));
}

// Minimum AVC level_idc needed to encode `w×h @ fps` (Annex A caps on
// MaxFrameSize and MaxMBPS). Returns the hex level_idc, e.g. 0x33 → L5.1.
function avcLevelNeeded(width, height, fps = 30) {
  const mbPerFrame = Math.ceil((width * height) / 256);
  const mbPerSec = mbPerFrame * Math.max(1, Math.round(fps));
  const caps = [
    { lvl: 0x28, fs: 8192, mbps: 245760 },   // 4.0
    { lvl: 0x29, fs: 8192, mbps: 245760 },   // 4.1
    { lvl: 0x2a, fs: 8704, mbps: 522240 },   // 4.2
    { lvl: 0x32, fs: 22080, mbps: 589824 },  // 5.0
    { lvl: 0x33, fs: 36864, mbps: 983040 },  // 5.1
    { lvl: 0x34, fs: 36864, mbps: 2073600 }, // 5.2
  ];
  for (const c of caps) if (mbPerFrame <= c.fs && mbPerSec <= c.mbps) return c.lvl;
  return 0x34;
}
// Sort candidates so the ones whose AVC level actually covers the frame come
// first, and among those prefer the lowest adequate level (most compatible).
function orderCodecs(codecs, need) {
  const lvl = (c) => parseInt(c.slice(-2), 16);
  const prof = (c) => (c.slice(5, 7) === "64" ? 0 : c.slice(5, 7) === "4d" ? 1 : 2); // High < Main < Baseline
  return [...codecs].sort((a, b) => {
    const oka = lvl(a) >= need ? 0 : 1, okb = lvl(b) >= need ? 0 : 1;
...
// Pick a supported H.264/AAC MP4 profile, capped to 4K (3840×2160). Returns null
// if the browser can't encode H.264 at ANY size (e.g. Firefox) — the caller then
// shows the unsupported-browser message. Hardware without a 4K encoder falls back
// to 1080p then 720p so a render still exports (the achieved size is in the
// returned profile; callers compare it against the requested resolution).
export async function pickRenderProfile(rawW, rawH, fps = 30, bitrate = crfTargetBitrate(rawW, rawH, fps)) {
  if (!webCodecsSupported()) return null;
  const even = (v) => Math.max(2, Math.round(v / 2) * 2);
  const fit = (w, h, maxW, maxH) => {
    const sc = Math.min(1, maxW / w, maxH / h);
    return { width: even(w * sc), height: even(h * sc) };
  };
  const codecs = ["avc1.640028", "avc1.4d0028", "avc1.640033", "avc1.640034", "avc1.42002a", "avc1.42001f"];
  const attempt = async (w, h, br) => {
    for (const codec of orderCodecs(codecs, avcLevelNeeded(w, h, fps))) {
      if (await videoCodecOk(codec, w, h, fps, br)) {
        // Prefer AAC (most compatible). If the browser can't encode AAC — some
        // Chromebooks / Chromium builds ship no AAC encoder, which was silently
        // producing soundless videos — fall back to Opus (royalty-free) muxed into
        // the same MP4. H.264 + Opus MP4 plays in Chrome, Android and ChromeOS.
  ...
  let p = await attempt(fit(rawW, rawH, 3840, 2160).width, fit(rawW, rawH, 3840, 2160).height, bitrate);
  if (!p) {
    const fb = fit(rawW, rawH, 1920, 1080);
    p = await attempt(fb.width, fb.height, crfTargetBitrate(fb.width, fb.height, fps));
  }
  if (!p) {
    const fb = fit(rawW, rawH, 1280, 720);
    p = await attempt(fb.width, fb.height, crfTargetBitrate(fb.width, fb.height, fps));
  }
  return p;
}
...
export async function renderWebCodecs(spec, imagesByName, onProgress, shouldCancel, logs = [], writable = null) {
  ...
  const { clips, fps = 30, transitions = [], transitionDuration = 0.4, motions = [], motionAmount = 0.08, fx = [], fxAmount = 0.5,
    audioFile = null,
    cues = null, captionStyle = "classic", captionSize = "md", captionLineHeight = 0, captionFontScale = 1, captionAnimation = "none",
    videosByName = {}, trims = [], speeds = [], volumes = [], bitrate = null, audioLayers = [], sfx = [],
    voiceFx = null,
    voiceLevel = 1,
    overlayUrl = null, overlayDuration = 0, overlayOpacity = 0.3, overlayBlendMode = "overlay", overlayLoop = true, overlayEnabled = false,
    watermarkUrl = null, watermarkSize = 0.15, watermarkX = 0.8, watermarkY = 0.88, watermarkOpacity = 0.9, watermarkEnabled = false,
    logoUrl = null, logoCorner = "br", logoSize = 0.12, logoOpacity = 0.9, logoEnabled = false,
    textOverlays = [], } = spec;
  ...
  const targetBitrate = bitrate || crfTargetBitrate(W, H, fps);
  ...
  const mixChannels = 2;
  // Voice-over effect processor (streaming — state persists across the read windows).
  const voiceFxProc = createVoiceFxProcessor(voiceFx, mixRate);
  ...
  // FX-lane sound effects (library presets or uploads) resolved to a URL by page.js.
  const sfxList = (sfx || []).filter((s) => s && s.url);
  const wantAudio = !!audioCodec && (!!voiceSource || ((clipAudio.length > 0 || sfxList.length > 0) && !!OAC));
  ...
  if (!audioCodec && (audioFile || clipAudio.length > 0 || sfxList.length > 0)) log("audio failed: this browser has no audio encoder (AAC or Opus) — the video will be silent");
  ...
  // Prepare audio layer mixes (background music, SFX, etc.) - now supports multiple clips per layer
  async function prepareLayerMixes() {
    const out = [];
    if (!audioLayers.length) return out;
    ...
    for (const layer of audioLayers) {
      ...
      for (const clip of layer.clips || []) {
        ...
        let cbuf = null;
        try { cbuf = await dctx.decodeAudioData(await clip.file.arrayBuffer()); }
        catch (_) { cbuf = null; }
        if (!cbuf) continue;
        const nCh = Math.min(mixChannels, cbuf.numberOfChannels);
        const startSample = 0; // audio clips start from beginning of file
        const regionLen = Math.min(cbuf.length - startSample, Math.round(clip.duration * mixRate));
        if (regionLen <= 0) continue;
        const region = [];
        for (let c = 0; c < nCh; c++) region.push(cbuf.getChannelData(c).subarray(startSample, startSample + regionLen));
        const chans = region.map((s) => Float32Array.from(s));
        out.push({ chans, at: Math.floor((clip.start || 0) * mixRate), vol: (clip.volume || 1) * (layer.volume || 1) });
      }
    }
    return out;
  }

  // Prepare FX-lane sound-effect mixes: fetch each resolved URL (a bundled preset
  // path or an upload's blob URL), decode the whole file, and place it at `at`
  // seconds at its own volume. Unlike a music layer these play once, uncut.
  async function prepareSfxMixes() {
    const out = [];
    if (!sfxList.length) return out;
    const dctx = new OAC(1, 1, mixRate);
    for (const s of sfxList) {
      let cbuf = null;
      try {
        const res = await fetch(s.url);
        cbuf = await dctx.decodeAudioData(await res.arrayBuffer());
      } catch (_) { cbuf = null; }
      if (!cbuf) continue;
      const nCh = Math.min(mixChannels, cbuf.numberOfChannels);
      // BG clips carry an in-point (`offset`) and play length (`duration`); plain
      // FX markers omit them and play the whole file.
      const startSample = Math.max(0, Math.round((s.offset || 0) * mixRate));
      const endSample = (s.duration == null)
        ? cbuf.length
        : Math.min(cbuf.length, startSample + Math.round(s.duration * mixRate));
      if (endSample <= startSample) continue;
      const chans = [];
      for (let c = 0; c < nCh; c++) {
        const data = cbuf.getChannelData(c);
        chans.push(Float32Array.from(data.subarray(startSample, endSample)));
      }
      // Apply the clip's fade-in/out envelope to the copied samples.
      const fi = Math.round((s.fadeIn || 0) * mixRate);
      const fo = Math.round((s.fadeOut || 0) * mixRate);
      if (fi > 0 || fo > 0) {
        const len = endSample - startSample;
        const fiN = Math.min(fi, len), foN = Math.min(fo, len);
        for (const arr of chans) {
          for (let i = 0; i < fiN; i++) arr[i] *= i / fiN;
          for (let i = 0; i < foN; i++) arr[len - 1 - i] *= i / foN;
        }
      }
      out.push({ chans, at: Math.max(0, Math.floor((s.at || 0) * mixRate)), vol: s.volume == null ? 0.8 : s.volume });
    }
    return out;
  }
  ...
  let overlayVideoCache = null; // overlay video element
  let watermarkImgCache = null; // watermark <img> (decoded once, reused every frame)
  let logoImgCache = null;      // corner logo <img> (decoded once, reused every frame)
  async function getWatermarkImg() {
    if (watermarkImgCache) return watermarkImgCache;
    if (!watermarkUrl) return null;
    const img = document.createElement("img");
    img.src = watermarkUrl;
    img.decoding = "async";
    await new Promise((res) => { img.onload = res; img.onerror = res; });
    if (img.width > 0) watermarkImgCache = img;
    return watermarkImgCache;
  }
  async function getLogoImg() {
    ...
    img.decoding = "async";
    await new Promise((res) => { img.onload = res; img.onerror = res; });
    if (img.width > 0) logoImgCache = img;
    return logoImgCache;
  }
  ...
  const fxCanvas = new OffscreenCanvas(W, H);
  ...
  const effBitrate = (streamToDisk || writable)
    ? targetBitrate
    : Math.max(2_000_000, Math.min(targetBitrate, Math.floor((memCap * 8) / Math.max(1, total))));
  ...
  log(`video ${W}x${H} @ ${fps}fps, ${total.toFixed(1)}s, ${totalFrames} frames, codec ${profile.videoCodec}, ${useQuantizer ? `quality mode (QP ${QP})` : `bitrate ${(effBitrate / 1e6).toFixed(1)} Mbps${effBitrate < targetBitrate ? " (capped for in-memory output)" : ""}`}`);
  ...
  let audioEnc = null, audioErr = null, audioOff = 0, aChunks = 0, audioDead = !wantAudio, clipMixes = [], layerMixes = [], sfxMixes = [];
  if (wantAudio) {
    try { clipMixes = await prepareClipMixes(); } catch (_) { clipMixes = []; }
    try { layerMixes = await prepareLayerMixes(); } catch (_) { layerMixes = []; }
    try { sfxMixes = await prepareSfxMixes(); } catch (_) { sfxMixes = []; }
    if (!voiceSource && !clipMixes.length && !layerMixes.length && !sfxMixes.length) {
      audioDead = true;
  ...
  async function pumpAudioTo(upto) {
    ...
    try {
      while (audioOff < upto) {
        ...
        const voLevel = Math.max(0, Math.min(1, voiceLevel == null ? 1 : +voiceLevel));
        if (voiceSource) {
          let vwin = await voiceSource.read(audioOff, n);
          if (voiceFxProc) vwin = voiceFxProc.process(vwin);
          for (let c = 0; c < channels; c++) {
            const src = vwin[Math.min(c, vwin.length - 1)];
            if (voLevel === 1) planar.set(src, c * n);
            else for (let i = 0; i < n; i++) planar[c * n + i] = src[i] * voLevel;
          }
        }
        ...
        for (const lm of layerMixes) {
          const cs = lm.at, ce = lm.at + lm.chans[0].length;
          if (ce <= audioOff || cs >= audioOff + n) continue;      // no overlap with [audioOff, audioOff+n)
          const lo = Math.max(audioOff, cs), hi = Math.min(audioOff + n, ce);
          for (let c = 0; c < channels; c++) {
            const s = lm.chans[Math.min(c, lm.chans.length - 1)];
            const base = c * n;
            for (let gi = lo; gi < hi; gi++) planar[base + (gi - audioOff)] += s[gi - cs] * lm.vol;
          }
        }
        for (const sm of sfxMixes) {
          const cs = sm.at, ce = sm.at + sm.chans[0].length;
          if (ce <= audioOff || cs >= audioOff + n) continue;      // no overlap with [audioOff, audioOff+n)
          const lo = Math.max(audioOff, cs), hi = Math.min(audioOff + n, ce);
          for (let c = 0; c < channels; c++) {
            const s = sm.chans[Math.min(c, sm.chans.length - 1)];
            const base = c * n;
            for (let gi = lo; gi < hi; gi++) planar[base + (gi - audioOff)] += s[gi - cs] * sm.vol;
          }
        }
  ...
  const transformAt = (i, tt) => {
    const c = clips[i];
    if (!c || c.gap) return { scale: 1, offsetX: 0, offsetY: 0, rotateX: 0, rotateY: 0 };
    const m = motions[i];
    if (!m || m === "none") return { scale: 1, offsetX: 0, offsetY: 0, rotateX: 0, rotateY: 0 };
    const lp = Math.min(1, Math.max(0, (tt - c.start) / c.duration));
    
    // Define motion effects inline for render (same as MOTION_EFFECTS in transitions.js)
    let tform = { scale: 1, offsetX: 0, offsetY: 0, rotateX: 0, rotateY: 0 };
    const eased = lp < 0.5 ? 2 * lp * lp : 1 - Math.pow(-2 * lp + 2, 2) / 2;
    const smooth = lp * lp * (3 - 2 * lp);
    
    // Deterministic pseudo-random based on clip index for consistent pans
    const seed = i * 1000;
    const randX = Math.sin(seed) * 0.5;
    const randY = Math.cos(seed) * 0.5;
    
    switch (m) {
      case "zoomin":
        tform = { scale: 1 + motionAmount * lp, offsetX: 0, offsetY: 0, rotateX: 0, rotateY: 0 };
        break;
      case "zoomout":
        tform = { scale: 1 + motionAmount * (1 - lp), offsetX: 0, offsetY: 0, rotateX: 0, rotateY: 0 };
        break;
      case "dynamic":
        tform = { 
          scale: 1 + motionAmount * eased, 
          offsetX: randX * 0.3 * (1 - eased) * W / 100, 
          offsetY: randY * 0.3 * (1 - eased) * H / 100, 
          rotateX: 0, rotateY: 0 
        };
        break;
      case "parallax":
        tform = { 
          scale: 1 + motionAmount * eased, 
          offsetX: randX * 0.4 * eased * 1.5 * W / 100, 
          offsetY: randY * 0.4 * eased * 1.5 * H / 100, 
          rotateX: 0, rotateY: 0 
        };
        break;
      case "camera3d":
        tform = { 
          scale: 1 + motionAmount * eased * 0.5, 
          offsetX: randX * 0.2 * eased * W / 100, 
          offsetY: randY * 0.2 * eased * H / 100, 
          rotateX: Math.sin(lp * Math.PI) * 0.15, 
          rotateY: Math.cos(lp * Math.PI) * 0.1 
        };
        break;
      case "morph":
        tform = { 
          scale: 1 + motionAmount * smooth, 
          offsetX: randX * 0.5 * (1 - smooth) * W / 100, 
          offsetY: randY * 0.5 * (1 - smooth) * H / 100, 
          rotateX: 0, rotateY: 0 
        };
        break;
      case "easedZoomBlur":
        {
          ...
          const scale = 1 + motionAmount * eased;
          const velocity = lp < 0.5 ? 12 * lp * lp : 12 * (1 - lp) * (1 - lp);
          const blurIntensity = Math.min(velocity * motionAmount * 20, 30);
          const panX = randX * 0.25 * (1 - eased) * W / 100;
          const panY = randY * 0.25 * (1 - eased) * H / 100;
          tform = { scale, offsetX: panX, offsetY: panY, rotateX: 0, rotateY: 0, blur: blurIntensity };
        }
        break;
      case "motionBlurZoom":
        {
          ...
          const blurIntensity = velocity * motionAmount * 25;
          const panX = randX * 0.4 * eased * W / 100;
          const panY = randY * 0.4 * eased * H / 100;
          const blurAngle = Math.atan2(panY, panX) * 180 / Math.PI;
          tform = { scale, offsetX: panX, offsetY: panY, rotateX: 0, rotateY: 0, blur: blurIntensity, blurAngle };
        }
        break;
      case "dollyZoom":
        {
          const eased = Math.sin(lp * Math.PI * 0.5);
          const scale = 1 + motionAmount * eased;
          const panX = -randX * 0.35 * eased * W / 100;
          const panY = -randY * 0.35 * eased * H / 100;
          tform = { scale, offsetX: panX, offsetY: panY, rotateX: 0, rotateY: 0 };
        }
        break;
      case "whipPanZoom":
        {
          ...
          if (lp < 0.15) {
            ...
            eased = 0.95 + (lp - 0.85) / 0.15 * 0.05;
            velocity = 0.05 / 0.15;
          }
          const scale = 1 + motionAmount * eased;
          const panX = randX * 0.5 * eased * W / 100;
          const panY = randY * 0.5 * eased * H / 100;
          const blurIntensity = velocity * motionAmount * 30;
          tform = { scale, offsetX: panX, offsetY: panY, rotateX: 0, rotateY: 0, blur: blurIntensity };
        }
        break;
      default:
        tform = { scale: 1, offsetX: 0, offsetY: 0, rotateX: 0, rotateY: 0 };
    }
    return tform;
  };
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    if (idx > 0 && tdur > 0 && t < clip.start + tdur) {
      ...
      transitionOf(type).canvas(ctx, prevFrame, curFrame, p, W, H, fromT.scale, toT.scale);
      ctx.globalAlpha = 1;
    } else {
      const frame = await getFrame(idx, t);
      const tform = transformAt(idx, t);
      if (frame) {
        const iw = frame.displayWidth || frame.videoWidth || frame.naturalWidth || frame.width;
        const ih = frame.displayHeight || frame.videoHeight || frame.naturalHeight || frame.height;
        if (iw && ih) {
    ...
    // Image effects: current clip's grade + overlays, burned in under the overlay
    // texture and captions — same pass the preview draw uses.
    const clipFx = (fx && fx[idx]) || "none";
    if (clipFx !== "none" && fxAmount > 0) applyFx(ctx, fxCtx, clipFx, fxAmount, W, H, fxSeed(clip.name), f);

    // Draw overlay video (old film texture, etc.) underneath the captions.
    if (overlayEnabled && overlayUrl) {
      // We'll use a cached video element for the overlay
      if (!overlayVideoCache) {
        overlayVideoCache = document.createElement("video");
        overlayVideoCache.src = overlayUrl;
        overlayVideoCache.muted = true;
        overlayVideoCache.loop = overlayLoop;
        overlayVideoCache.playsInline = true;
        overlayVideoCache.preload = "auto";
        await new Promise((resolve) => {
          overlayVideoCache.oncanplay = resolve;
          overlayVideoCache.load();
        });
      }
      const ov = overlayVideoCache;
      if (ov.readyState >= 2 && ov.videoWidth && ov.videoHeight) {
        // Time-sync the overlay to the playhead so the texture actually animates.
        const dur = overlayDuration > 0 ? overlayDuration : (ov.duration || 1);
        const target = overlayLoop ? (t % dur) : Math.min(t, dur);
        if (Math.abs(ov.currentTime - target) > 0.08) {
          try { ov.currentTime = target; } catch (_) {}
        }
        if (ov.paused) { try { ov.play().catch(() => {}); } catch (_) {} }
        ctx.save();
    ...
    // Captions drawn on top of the images and overlay.
    if (hasCaptions) {
      const cue = captionCueAt(cues, t);
      if (cue) drawCaption(ctx, cue.text, W, H, captionStyle, capFontPx, captionLineHeight, captionAnimation, t - cue.start, cue.end - cue.start);
    }

    // Timed text overlays (titles/labels), same layer as the canvas preview.
    if (Array.isArray(textOverlays) && textOverlays.length) drawTextOverlays(ctx, textOverlays, W, H, t);

    // Static image overlay (logo / watermark) — the top-most layer, like the ffmpeg render.
    if (watermarkEnabled && watermarkUrl) {
      const logo = await getWatermarkImg();
      if (logo) drawWatermark(ctx, logo, W, H, { size: watermarkSize, x: watermarkX, y: watermarkY, opacity: watermarkOpacity });
    }

    // Corner logo overlay — the very top-most layer (above the watermark).
    if (logoEnabled && logoUrl) {
      const lg = await getLogoImg();
      if (lg) drawCornerLogo(ctx, lg, W, H, { corner: logoCorner, size: logoSize, opacity: logoOpacity });
    }
